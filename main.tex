...
At any rate, the problem of efficiently "reading" quantum information encoded in amplitudes, which is common in Quantum Machine Learning applications, is still not fully resolved.
...
\section{Discussions}
QPCA is an important tool in the field of Quantum ML, whose implementation in its original formulation however presents some difficulties.
Until a practical QRAM model is available, procedures to prepare a state encoding the covariance matrix of a quantumly accessible dataset seem hardly feasible.
Nevertheless, prior works have shown that the algorithm can be adapted to work with any initial state, as long as the matrix can be encoded in the unitary operators.
\todo[color=green]{does this first part belong to introduction?}
...
Then, as discussed, decoding information stored in the amplitudes of a quantum state is not an easy task, and solutions to do it in a way that preserves the potential speed-up achieved by the algorithm are still being researched.
...
% missing comment on experiments
