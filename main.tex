...
We consider the case in which the solution is to be decoded into classical form, and describe a method that through measurements allows to obtain the sought after state encoding the principal subspace when starting from a random mixed state.
...
arxiv.org/pdf/1810.10710.pdf, use Lloyd's qpca method to map a set of data points stored in quantum form directly in the lower dimensional principal subspace.

arxiv.org/pdf/1512.02109.pdf, propose a variant to Lloyd's qpca that starts from an initial superposition state and uses amplitude amplification to boost the desired eigenvalues. Number of iterations of the amplitude amplification is critical as can lead to reduce the probability of the desired eigenvalues instead.

arxiv.org/pdf/1811.00414.pdf, discuss a quantum-inspired classical algorithm model and use it to de-quantize Lloyd's qpca algorithm.
...
Other methods using different and more convenient input models have been proposed, like the one of \cite{wang2018quantum}  based on the qRAM model and with time complexity $\mathcal{\Tilde{O}}(\sqrt{d})$.
